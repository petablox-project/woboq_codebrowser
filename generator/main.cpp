...
cl::opt<std::string> BuildPath(
    "b", cl::value_desc("compile_commands.json"),
    cl::desc("Path to the compilation database (compile_commands.json) If this "
             "argument is not passed, the compilation arguments can be passed "
...
cl::list<std::string> ProjectPaths(
    ...
    cl::desc("Project specification: The name of the project, the absolute "
...
cl::opt<std::string> DataPath(
    "d", cl::value_desc("data path"),
    cl::desc(
        "Data url where all the javascript and css files are found. Can be "
        "absolute, or relative to the output directory. Defaults to ../data"),
    cl::Optional);

cl::opt<bool> ProcessAllSources(
    "a",
    cl::desc(
        "Process all files from the compile_commands.json. If this argument is "
        "passed, the list of sources does not need to be passed"));
...
enum class DatabaseType { InDatabase, NotInDatabase, ProcessFullDirectory };
...
struct BrowserDiagnosticClient : clang::DiagnosticConsumer {
  ...
  virtual void HandleDiagnostic(clang::DiagnosticsEngine::Level DiagLevel,
    ...
    case clang::DiagnosticsEngine::Error:
      std::cerr << "Error: "
                << locationToString(Info.getLocation(),
...
public:
  BrowserASTConsumer(clang::CompilerInstance &ci,
                     ProjectManager &projectManager, DatabaseType WasInDatabase)
  ...
  virtual void HandleTranslationUnit(clang::ASTContext &Ctx) override {
    ...
    annotator.generate(ci.getSema(),
                       WasInDatabase != DatabaseType::NotInDatabase);
  }
...
static bool proceedCommand(std::vector<std::string> command,
                           llvm::StringRef Directory, llvm::StringRef file,
                           clang::FileManager *FM, DatabaseType WasInDatabase) {
  ...
  auto Ajust = [&](clang::tooling::ArgumentsAdjuster &&aj) {
    command = aj.Adjust(command);
  };
  ...
  clang::tooling::ToolInvocation Inv(command, new BrowserAction(WasInDatabase),
                                     FM);
...
int main(int argc, const char **argv) {
  ...
  if (Sources.empty() && ProcessAllSources) {
    ...
    while (DirName.endswith("/"))
      DirName.pop_back();
    std::error_code EC;
    for (llvm::sys::fs::recursive_directory_iterator it(DirName.str(), EC),
         DirEnd;
...
#else
    std::cerr << "Passing directory is only implemented with llvm >= 3.5"
              << std::endl;
    return EXIT_FAILURE;
#endif
  }
  ...
  if (ProjectPaths.empty() && !IsProcessingAllDirectory) {
    std::cerr << "You must specify a project name and directory with '-p "
                 "name:directory'"
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  for (const auto &it : Sources) {
    ...
    if (auto project = projectManager.projectForFile(filename)) {
      ...
      std::cerr << "Sources: Skipping file not included by any project "
    ...
    if (!compileCommandsForFile.empty() && !isHeader) {
      std::cerr << '[' << (100 * Progress / Sources.size()) << "%] Processing "
                << file << "\n";
  ...
  for (const auto &it : NotInDB) {
    ...
    if (auto project = projectManager.projectForFile(file)) {
      if (!projectManager.shouldProcess(file, project)) {
        std::cerr << "NotInDB: Skipping already processed " << file.c_str()
                  << std::endl;
        continue;
      }
    } else {
      std::cerr << "NotInDB: Skipping file not included by any project "
                << file.c_str() << std::endl;
      continue;
    }
    ...
    if (!compileCommandsForFile.empty()) {
      ...
      if (llvm::StringRef(file).endswith(".qdoc")) {
        ...
        command.push_back(llvm::StringRef(file).substr(0, file.size() - 5) %
                          ".h");
      }
      success = proceedCommand(
          std::move(command), compileCommandsForFile.front().Directory, file,
          &FM,
