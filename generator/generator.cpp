...
void Generator::getCoveredLines(std::vector<int> &coveredLines,
                                std::string cov_filename, std::string filename) {
  std::ifstream file(cov_filename);
  int a;
  ...
  while (std::getline(file, line)) {
    ...
    bool number = (bool) (iss >> a);

    if(!number  && line.find(filename) != std::string::npos){
	foundFile = true;
    }else if(!number){
	foundFile = false;
    }
...
void Generator::generate(llvm::StringRef outputPrefix, std::string dataPath,
  ...
  getCoveredLines(coveredLines, "lines_covered", exactFilename);
  ...
  myfile << "<br />Powered by <a href='https://woboq.com'><img alt='Woboq' "
            "src='https://code.woboq.org/woboq-16.png' width='41' height='16' "
            "/></a> <a href='https://code.woboq.org'>Code "
            "Browser</a> " CODEBROWSER_VERSION
            "\n<br/>Generator usage only permitted with "
            "license.</p>\n</div></body></html>\n";
}
